...
Let's verify that with the simplest piece of code we can think of:

![simple fennel](/assets/fennel/raw_fennel.png)

I just pressed `\ee` (see [Conjure doc](TODO) to learn the basics of using Conjure) on the form to eval, and hop! It appears to be working like a charm. Look, it even prints my text in vim's message buffer at the bottom. We're getting closer to hacking vim now!
...
So don't worry if like me, you're not particularly familiar with the process of shipping and loading Lua files, for now at least, because we'd like to avoid doing this in the process of developping our plugin.
